...
class Reseller(Investment, Emitter):
    ...
    def __init__(self, supply_chain, transport_parameter, emission_factor):
        ...
        self.activity_level = after_invest(
            supply_chain.transport_tkm(), self.parameter.time_horizon
        )
    ...
    def driving_work(self):
        """Return the hours of work used to transport the biomass from fields to the plant.

        The integral calculus is done when computing self.activity_level.
        """
        time = (
            self.activity_level
            / self.parameter.truck_load
            / self.parameter.truck_velocity
        )
        return display_as(time, "hr")
    ...
    def labor_cost(self):
        """Return the cost of work for loading and transporting the biomass."""
        amount = self.loading_wages() + self.driving_wages()
        return display_as(amount, "kUSD")

    def fuel_cost(self):
        """Return the cost of fuel used for loading and transporting the biomass."""
        amount = (
            self.driving_work() * self.parameter.fuel_cost_per_hour_driving
    ...
    def operating_expenses_detail(self):
        ...
        expenses_index = ["Truck rental", "Truck fuel", "Handling work", "Driving work"]
